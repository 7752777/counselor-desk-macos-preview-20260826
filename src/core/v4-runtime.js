(function installV4Runtime(global) {
  ...
  // IndexedDB's physical schema version is separate from the workspace
  // protocol/data schema. Increment it whenever a new object store is added
  // so existing browser workspaces actually receive the current manifest.
  ...
  function createMemoryRepository(name) {
    ...
    return {
      ...
      async putMany(items) {
        ...
        values.forEach(value => records.set(String(value.id), value));
        return values;
      },
      async replaceManyAtomic(items, options) {
        if (!Array.isArray(items)) throw new Error('REPOSITORY_RECORDS_INVALID');
        const assumeCloned = !!(options && options.assumeCloned === true);
        const values = items.map(value => {
          if (!value || !value.id) throw new Error('REPOSITORY_ID_REQUIRED');
          return assumeCloned ? value : clone(value);
        });
        const bulkKey = options && typeof options.bulkKey === 'string' ? options.bulkKey.trim() : '';
        records.clear();
        if (bulkKey) records.set(bulkKey, { id:bulkKey, records:values, schema_version:V4_SCHEMA_VERSION, created_at:new Date().toISOString(), updated_at:new Date().toISOString() });
        else values.forEach(value => records.set(String(value.id), value));
        return values;
      },
  ...
  function createIndexedDbRepository(name, dbName) {
    ...
    const open = () => {
      ...
      databasePromise = new Promise((resolve, reject) => {
        const legacySources = [];
        // Schema v8 and later collections are additive. The physical IDB
        // version must advance independently from the workspace schema so an
        // existing v5 database creates all newly introduced object stores.
        const request = global.indexedDB.open(databaseName, INDEXED_DB_VERSION);
    ...
    return {
      ...
      replaceManyAtomic: (records, options) => {
        if (!Array.isArray(records)) return Promise.reject(new Error('REPOSITORY_RECORDS_INVALID'));
        ...
        const bulkKey = options && typeof options.bulkKey === 'string' ? options.bulkKey.trim() : '';
        const storedValues = bulkKey && values.length
          ? [{ id:bulkKey, schema_version:V4_SCHEMA_VERSION, records_json:JSON.stringify(values), created_at:new Date().toISOString(), updated_at:new Date().toISOString() }]
          : values;
        return (async () => {
          const db = await open();
          await new Promise((resolve, reject) => {
            ...
            try {
              ...
              clear.onsuccess = () => queueAtomicReplaceBatches(objectStore, storedValues, bulkKey ? storedValues.length : 64, error => {
