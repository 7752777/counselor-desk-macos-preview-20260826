...
(async () => {
  ...
  await sleep(500);
  if (w.CWB && w.CWB.security && !w.CWB.security.isEnabled()) w.CWB.security.configure('1234');
  ...
  const confirmSensitiveExport = async () => {
    await sleep(20);
    const confirm = $('#modal-root [data-sensitive-export-confirm]');
    if (confirm) { click(confirm); await sleep(40); }
  };
  ...
  ok('事件分发器把按钮元素透传给动作（支持 data-coll）', /ACTS\[action\]\(button\.dataset\.id,\s*button\)/.test(src) || /ACTS\[btn\.dataset\.act\]\(btn\.dataset\.id,\s*btn\)/.test(src));
