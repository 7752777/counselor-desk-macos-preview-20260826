...
(async () => {
  ...
  if (cwb.version === expectedVersion) ok(`APP_VERSION = ${expectedVersion}`);
  else bad('APP_VERSION 错误：' + cwb.version);

  const brandSub = $1('#brand-sub');
  if (brandSub && !brandSub.textContent.includes('低空学院') && /(开源版|开发模式|商业版 · AI 待激活|本地工作区)/.test(brandSub.textContent)) ok('品牌副标适用于全国高校和商业授权状态');
