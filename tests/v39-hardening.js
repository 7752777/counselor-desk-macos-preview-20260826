...
(async () => {
  ...
  assert.equal(d.title, '学工智伴 v4.9.0', 'browser title should use the current public product name and version');
  ...
  const originalInspect = w.CWB.storage.inspect;
  w.CWB.storage.inspect = async () => ({ used:4_500_000, capacity:5_000_000, ratio:.9, level:'critical', attachments:{ count:0, bytes:0 } });
  await assert.rejects(
    w.CWB.theme.prepareBackgroundFile({ type:'image/jpeg', size:100, name:'small.jpg' }),
    /STORAGE_CAPACITY_LOW/
  );
  w.CWB.storage.inspect = originalInspect;
  ...
  const releaseFile = path.join(tempDir, '学工智伴-v4.9.0.html');
  try {
