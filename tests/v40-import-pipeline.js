...
(async () => {
  ...
  assert.equal(await cwb.importer.undoAsync(manualCommit.runId), true, 'import undo should persist the restored collection before reporting success');
  assert.equal((await cwb.repositories.students.list()).filter(student => student.student_number === 'MANUAL-1').length, 0, 'import undo should remove the imported student after reopening the repository');
  assert.equal((await cwb.importer.getHistoryAsync()).some(item => item.id === manualCommit.runId), false, 'successful import undo should remove only the selected snapshot');
  dom.window.close();
